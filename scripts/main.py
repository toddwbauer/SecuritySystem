...
PIR_PIN = 8             # pin the PIR is connected to on the Raspberry Pi
...
# Displays the video feed from the stream() function to the web server.
...
def detect():
    ...
    while True:
        ...
        # (Note that this is similar to the video recording in generate())
        if pir.detect_motion() is True:
            if motion_is_ongoing is False:
                ...
                if data_logger.motion_log_enabled is True:
                    ...
                    ###########################################
                    #!!!!SEND PHONE NOTIFICATION GOES HERE!!!!#
                    ###########################################
        else:
...
#################################################################
# Streams the frames generated by generate() to the web server. #
#################################################################
def stream():
    while True:
        with lock:
...
if __name__ == '__main__':
    ...
    camThread = threading.Thread(target=generate)
    camThread.daemon = True
    camThread.start()
    
    # Start a thread to detect motion every cycle.
    pirThread = threading.Thread(target=detect)
    ...
    refreshThread = threading.Thread(target=record)
    refreshThread.daemon = True
    refreshThread.start()
